...
2. 逐条 **Add（添加）** 下面几项（`VAPID_PRIVATE_KEY` 建议选类型 **Secret（加密）**，其余普通文本即可）：
   ...
   | `VAPID_SUBJECT` | `mailto:你的邮箱`（随便一个能联系到你的邮箱，例如mailto:youremail@gmail.com，注意一定要加`mailto:`，否则ios发送会403） |
...
- **状态码 403**：VAPID 公钥/私钥不是同一对，或者邮箱没有加`mailto:`前缀。确认 App 公钥、Worker 的 `VAPID_PUBLIC_KEY`、`VAPID_PRIVATE_KEY` 来自同一次生成；改完在 App 里关开关重开以重新订阅。
