...
try {
    // 使用学习功能侧边栏选定的预设（与 study_ai.js 共享 _getStudyApiConfig）
    const cfg = _getStudyApiConfig();

    // 构建世界书 system prompt（原有逻辑保留）
    const globalWorldBookIds = db.pomodoroSettings?.globalWorldBookIds || [];
    const wbBefore = globalWorldBookIds
        .map(id => db.worldBooks.find(wb => wb.id === id && wb.position === 'before'))
        .filter(Boolean).map(wb => wb.content).join('\n\n');
    const wbAfter = globalWorldBookIds
        .map(id => db.worldBooks.find(wb => wb.id === id && wb.position === 'after'))
        .filter(Boolean).map(wb => wb.content).join('\n\n');

    let systemPrompt = `你正在扮演角色。你的名字是${character.realName}。`;
    if (wbBefore) systemPrompt += `\n\n【全局世界观设定】\n${wbBefore}`;
    systemPrompt += `\n\n【你的角色设定】\n人设: ${character.persona}`;
    if (wbAfter)  systemPrompt += `\n\n【补充设定】\n${wbAfter}`;
    systemPrompt += `\n\n【我的角色设定】\n我的名字是${userName}，人设是：${userPersona}。`;

    // 是否启用流式（取决于预设配置）
    const useStream = cfg.streamEnabled !== false;

    if (useStream) {
        // 流式：清空 typing indicator，实时 append
        messageP.textContent = '';
        await callAI(prompt, {
            systemPrompt,
            onStream: (chunk) => { messageP.textContent += chunk; }
        });
    } else {
    ...
    // 获取最终文本用于写入历史
    const finalReply = messageP.textContent;
    pomodoroSessionHistory.push({ type: 'user', content: promptType });
    pomodoroSessionHistory.push({ type: 'ai',   content: finalReply });
    if (pomodoroSessionHistory.length > 8) pomodoroSessionHistory.splice(0, 2);

    if (promptType === 'poke') {
        setTimeout(() => { pomodoroIsInterrupted = false; }, 10000);
    }

} catch (error) {
    console.error('获取AI回应失败:', error);
    messageP.textContent = error.message || '获取回应失败，请检查网络或API设置。';
}
                }
