...
window.loadData = async () => {
    try {
        ...
        const[
            ...
            newPeekData,
            // ★★★ 不再全量读取 messages 表（12w+ 条会 OOM 崩溃）
            // 消息改为"按需加载"：openChatRoom 时只读最近 N 条进 chat.history
            // 列表预览/排序/消息数改读冗余字段 chat.lastMsgPreview / chat.msgCount
            // 搜索/总结/撤回追溯等场景直接打 Dexie 查询（见 history_window.js）
    ...
    newStudyBookSummaries,
            ...
            // ★ 删除 dexieDB.messages.toArray() —— 这是崩溃元凶
    ...
    dexieDB.studyBookSummaries.toArray(),
        ...
        // ★★★ 消息不再全量挂回内存：每个 chat 初始化为空 history，按需加载 ★★★
        // 真正的消息只在 openChatRoom 时通过 loadRecentMessages 读最近 N 条进 chat.history
        // 列表预览/排序/消息数改读冗余字段 chat.lastMsgPreview / chat.msgCount
        // 搜索/总结/撤回追溯等场景直接打 Dexie 查询（见 history_window.js）
        characters.forEach(c => { if (!Array.isArray(c.history)) c.history = []; });
        groups.forEach(g     => { if (!Array.isArray(g.history)) g.history = []; });

        // ★ 一次性迁移：给缺失 msgCount/lastMsgPreview 的 chat 回填冗余字段
        // 老用户首次升级到本次重构版本时触发，之后零成本（字段会随 char 表读写）
        const _needMeta = [...characters, ...groups].filter(c =>
            typeof c.msgCount !== 'number' || !c.lastMsgPreview
        );
        if (_needMeta.length > 0) {
            console.log(`📦 首次消息冗余字段迁移：${_needMeta.length} 个会话需要回填...`);
            const _migrateOne = async (chat) => {
                try {
                    const cnt = await dexieDB.messages.where('chatId').equals(chat.id).count();
                    chat.msgCount = cnt;
                    if (cnt > 0) {
                        const last = await dexieDB.messages.where('chatId').equals(chat.id).reverse().limit(1).first();
                        if (last) {
                            chat.lastMsgPreview = {
                                content: last.content, timestamp: last.timestamp || 0,
                                role: last.role, isUserStatusNotif: !!last.isUserStatusNotif, id: last.id
                            };
                        } else { chat.lastMsgPreview = null; }
                    } else { chat.lastMsgPreview = null; }
                } catch (e) { console.warn('⚠️ 冗余字段回填失败:', chat.id, e); }
            };
            // 串行避免 IndexedDB 过载；老会话不多时很快
            for (const c of _needMeta) { await _migrateOne(c); }
            console.log('✅ 消息冗余字段迁移完成');
            // 立即写回表，下次启动免迁移
            try { await saveData(); } catch(e) { console.warn('⚠️ 迁移后写回失败:', e); }
        }
...
window.saveMessageToDB = async (msg, chatId, chatType) => {
    // ★ 修复：失败时不再静默吞错，必须 throw 让调用方感知
    // 否则会出现“内存有 db 没有”的幽灵消息，重启后丢失
    try {
        await dexieDB.messages.put({ ...msg, chatId, chatType });
        // ★ 同步冗余字段（msgCount + lastMsgPreview），供列表预览/排序/消息数读取
        if (typeof _refreshChatMeta === 'function') {
            _refreshChatMeta(chatId, chatType, msg).catch(e => console.warn('⚠️ meta refresh 失败:', e));
        }
    } catch (e) {
        ...
        throw e;  // ★ 关键：让上层感知，触发回滚/重试
    }
};
window.saveMessagesToDB = async (msgs, chatId, chatType) => {
    try {
        await dexieDB.messages.bulkPut(msgs.map(m => ({ ...m, chatId, chatType })));
        // ★ 同步冗余字段：批量写后以最后一条作为 lastMsgPreview
        if (typeof _refreshChatMeta === 'function' && msgs.length > 0) {
            const last = msgs[msgs.length - 1];
            _refreshChatMeta(chatId, chatType, last).catch(e => console.warn('⚠️ meta refresh 失败:', e));
        }
    } catch (e) {
...
window.deleteMessagesFromDB = async (msgIds) => {
    try {
        ...
        // ★ 删除后冗余字段无法精准定位影响哪个 chat，统一刷新所有受影响的
        // （删除是低频操作，刷新成本可接受）
        if (typeof _refreshChatMeta === 'function') {
            // 通过 msgId 反查 chatId 不现实（已删），改为全量刷新所有 chat 的 lastMsgPreview
            const allChats = [...(db.characters || []), ...(db.groups || [])];
            for (const c of allChats) {
                _refreshChatMeta(c.id, c.type || (db.characters.includes(c) ? 'private' : 'group'), null)
                    .catch(e => console.warn('⚠️ meta refresh 失败:', e));
            }
        }
