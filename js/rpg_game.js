...
const RPG_DROP_CONFIG = {
    ...
    normal: [
        ...
        { id: 'potion_revive', rate: 0.05, min: 1, max: 1,  name: '复活药水' }, // 5% 掉蓝药
        { id: 'potion_purify', rate: 0.05, min: 1, max: 1,  name: '净化药水' } // 5% 掉蓝药
    ],
    
    // BOSS 掉落列表
    boss: [
        { id: 'currency',    rate: 1.0,  min: 50, max: 100, name: '' },      // 100% 掉 100-150 金币
        { id: 'return_scroll', rate: 0.5, min: 1, max: 1, name: '回家卷轴' },
        { id: 'dye',         rate: 0.1,  min: 1,   max: 1,   name: '染料' },  // 80% 掉染料
        { id: 'blueprint',   rate: 0.3,  min: 1,   max: 1,   name: '图纸' },  // 30% 掉图纸
...
// === 通用的绘制逻辑，用于 P1 和 P2 ===
    const drawCroppedPreview = (canvas, settings) => {
        ...
        // 1. 生成原始 256x256 SpriteSheet
        const sheet = generateCharaSprite(settings);

        // 2. 设置 Canvas 内部分辨率（长方形，高分辨率以保持清晰）
        // 设为 64宽 x 128高，这样是 1:2 的长方形
        canvas.width = 64;  
        canvas.height = 64; 

        // 3. 裁剪并绘制
        // 源图(sheet): 截取左上角人物 (x=0, y=0)，原始大小是 64x64
        // 我们想只看中间部分（去掉左右空白，去掉头顶过多留白）
        // 假设人物主体在 64x64 的中心区域：x:16, y:8, w:32, h:48
        
        // drawImage(img, sx, sy, sw, sh, dx, dy, dw, dh)
        // 目标：放大显示到 canvas.width, canvas.height
        
        // 方案：截取人物主体 (x=12, y=4, w=40, h=56) 并放大铺满
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(
            sheet, 
            0, 0, 64, 64,      // 源截取：稍微修剪四周空白
            0, 0, 64, 64       // 目标：拉伸到整个长方形画布 (像素风拉伸不影响观感，或者你可以按比例)
        );
        // 或者保持比例居中放大：
        // ctx.drawImage(sheet, 0, 0, 64, 64, -32, 20, 128, 128); // 放大2倍并居中
    };

// --- 辅助工具：颜色变亮/变暗 ---
// col: hex颜色, amt: 数值 (正数变亮，负数变暗)
...
function generateCharaSprite(settings) {
    // 兼容旧调用：如果只传了颜色，或者结构不对，进行转换
    // 默认值处理
    const S = {
        hair: settings.hair || '#917C66',
        eye: settings.eye || '#D7CAFF',
        skin: settings.skin || '#FFE4CF',
        top: settings.top || '#3498db',     // 上衣颜色 (原 bodyColor)
        bottom: settings.bottom || '#2c3e50', // 下装颜色
        shoe: settings.shoe || '#101018',   // 鞋子颜色
        bangs: settings.bangs !== undefined ? parseInt(settings.bangs) : 0, // 0:斜刘海, 1:中分, 2:齐刘海
        back: settings.back !== undefined ? parseInt(settings.back) : 0,    // 0:无, 1:中发, 2:长发
        botStyle: settings.botStyle !== undefined ? parseInt(settings.botStyle) : 0 // 0:裤子, 1:裙子
    };
    ...
    const P = {
        ...
        'M': adjust(S.skin, -80), 
        'Y': adjust(S.skin, -40),
        'E': adjust(S.eye, -60), // 瞳孔
        'A': S.eye, // 颜色主色，虹膜 (复用眼睛色系)
        'W': "#ffffff", // 眼白和高光
        'O': "#4a3c31", // 睫毛        
        'G': "#cccccc", // 眼球阴影
        'F': "#ffb0a0", // 红晕
