...
// ── RPG 专属 API 预设选择 ──
window.rpgSelectedApiPreset = null; // null = 使用全局默认
...
const apiSettingBtn = document.getElementById('rpg-api-setting-btn');
if (apiSettingBtn) apiSettingBtn.onclick = () => rpgShowApiPickerDialog();
...
/** 复用现有 #app-global-dialog 实现预设选择弹框 */
function rpgShowApiPickerDialog() {
    return new Promise((resolve) => {
        ...
        if (!overlay) return resolve(undefined);

        titleEl.innerText = 'API 预设';
        ...
        sel.id = 'rpg-api-preset-picker';
        sel.style.cssText = 'width:100%;padding:10px 8px;border-radius:10px;border:1px solid #ddd;font-size:14px;margin-top:4px;background:#f7f7f7;';

        const defaultOpt = document.createElement('option');
        defaultOpt.value = '';
        defaultOpt.textContent = '全局默认';
        ...
        chatPresets.forEach(p => {
            ...
            if (window.rpgSelectedApiPreset === p.name) opt.selected = true;
            sel.appendChild(opt);
        });

        if (!window.rpgSelectedApiPreset) sel.value = '';
        contentEl.innerHTML = '';
        ...
        actionsEl.appendChild(createBtn('确定', 'btn-primary', () => {
            window.rpgSelectedApiPreset = sel.value || null;
            const label = sel.value ? `已选：${sel.value}` : '已选：全局默认';
            showToast(label);
            resolve(window.rpgSelectedApiPreset);
        }));
        actionsEl.appendChild(createBtn('取消', 'btn-neutral', () => resolve(undefined)));
