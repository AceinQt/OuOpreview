...
// ★ 改动请求体的拼法之后，把这个版本号 +1 —— 同一段文字会合成出不同的音频，
//   旧缓存必须整体失效。这是唯一的失效手段，别指望别处会自动处理。
// v1 → v2：key 从"预设内容指纹"改成"预设 id"（见 computeVoiceKey）。
//
// ★ 加 MiniMax、去掉「声音描述」那次**刻意没有**顶版本号。
//   去描述之后同一句话的豆包发音确实和以前不同，但已合成的音频本来也没坏，
//   顶版本会让全部存量音频作废重花一次钱。代价是同一角色新旧语音的语气可能
//   略有差异 —— 这是已知且可接受的不一致，不好听的那条用气泡菜单里的
//   「重新生成」单条处理即可。
 ...
 *   `presetId` 是"哪个声音"的身份，预设里的服务商 / 音色 ID / 语速都是可变的配置。
