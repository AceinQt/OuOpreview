...
function setupMemoryJournalScreen() {
    ...
    generateForm.addEventListener('submit', async (e) => {
        ...
        const generateBoth = (currentMemoryTab === 'summary' && bothSwitch.checked);
    ...
    bindWorldBookBtn.addEventListener('click', () => {
        ...
        const currentBoundIds = currentMemoryTab === 'summary' 
            ? (character.summaryWorldBookIds || []) 
            : (character.journalWorldBookIds || []);
    ...
    saveWorldBookBtn.addEventListener('click', async () => {
        ...
        if (currentMemoryTab === 'summary') {
            character.summaryWorldBookIds = selectedIds;
    ...
    journalListContainer.addEventListener('click', async (e) => {
         ...
         const item = character[targetArrayName].find(j => j.id === id);
         if (!item) return;

         // 删除
         if (target.closest('.delete-journal-btn')) {
             if (confirm('确定要删除这条记录吗？')) {
                 character[targetArrayName] = character[targetArrayName].filter(j => j.id !== id);
                 // 如果是删除短期总结，还要考虑长期总结的关联 (可选，此处暂不处理)
                 await saveData();
                 renderMemoryScreen();
                 showToast('已删除');
             }
    ...
    editSummaryBtn.addEventListener('click', async () => {
        ...
        if (isEditing) {
            ...
            // 总结页可能是短期或长期，需再次判断
            let item;
            if (currentSummarySubTab === 'long') item = character.longTermSummaries.find(j => j.id === currentJournalDetailId);
            else item = character.memorySummaries.find(j => j.id === currentJournalDetailId);

            if (item) {
                item.title = summaryTitleEl.textContent.trim();
                item.content = summaryContentEl.innerText; 
                item.occurredAt = summaryDateInput.value.trim(); // 长期总结通常只读，但也支持改
                await saveData();
                showToast('保存成功');
                renderMemoryScreen();
            }
            ...
            if (item) summaryContentEl.innerText = item.content; // 源码
...
function renderMemoryScreen() {
    ...
    if (currentMemoryTab === 'summary') {
        if (currentSummarySubTab === 'long') {
            items = character.longTermSummaries || [];
            placeholderText.textContent = '还没有长期总结哦~';
        } else {
            items = character.memorySummaries || [];
            placeholderText.textContent = '还没有短期总结哦~';
        }
    } else {
        items = character.memoryJournals || [];
