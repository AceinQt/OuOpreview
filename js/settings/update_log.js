const appVersion = "Q.1.9"; // Current app version
            const updateLog = [
            {
                    version: "Q.1.9",
                    date: "2026-07-27", // 注：日期占位，您可以修改为实际发布的日期
                    notes: [
                        "✨ 新功能",
                        "1.系统返回功能",
                        "(1)默认关闭，可在“设置”页面开启（滑动返回功能下方）；",
                        "(2)开启后，点击系统导航栏的返回键不再退出，而是返回上一页面，仅在home页点击系统返回键才触发退出。",
                        "2.聊天回复条数设置",
                        "(1)设置路径在聊天室右上角开启侧边栏“回复条数”，点击输入回复条数范围。置空则为原来的默认值；",
                        "(2)本设置仅修改了发给ai的提示词，而不是程序去控制，如果ai犯蠢可能会不遵循你的设置；",
                        "(3)仅适用于线上模式。",
                        "3.聊天图片转文字功能",
                        "(1)增加长按菜单“转文字”按钮。发送图片后，可长按图片转化为文字图片，转化不可逆，转化后原图片就从存储里消失了；",
                        "(2)默认使用聊天api多调用一次识图，但可在聊天列表右上角设置栏设置识图用的api，这个api设置后，所有聊天中图片转化功能都会用设置的api；",
                        "(3)增加聊天设置栏“图片清理（转文字）”功能，点击后可批量把聊天中的图片转文字图片。部分api可能有每分钟调用次数限制，失败的话需要多试几次。也可以直接搜索base查到图片后一张一张转化。",
                        "🌱 功能优化",
                        "1.论坛优化",
                        "(1)支持帖子分享到群聊；",
                        "(2)支持关联群聊聊天记录；",
                        "(3)优化论坛user人设读取，现在会跟随聊天设定变化了（但需要重新绑定一次才能生效）；",
                        "(4)优化论坛内容切分逻辑，减少评论进入正文的概率。",
                        "2.优化存储",
                        "(1)进一步减少论坛内存占用，但因此会增加部分功能调用时长，本次增加论坛“我”页面数据加载时间；",
                        "(2)“存储备份”页面可查看查手机数据，但仍保持和角色数据一起导出的设置。同时优化查手机页面加载，避免卡顿。",
                        "3.查手机功能",
                        "(1)支持一键生成所有app内容（每个仅一条）；",
                        "(2)优化相册显示，现在按日期分隔；",
                        "(3)新增查手机消息继续演绎功能，遇到有趣的char和npc的消息，点击右上角的刷新按钮，就可以继续生成了。",
                        "🔧 问题修复",
                        "1.修复存量短期总结未更新问题，重复存在的短期总结可通过“存储备份”页面底部的清除重复数据进行清理；",
                        "2.修复未绑定人设的旧版聊天无法编辑人设问题；",
                        "3.修复主动话题消息重复发送的问题；",
                        "4.修复长按菜单闪烁问题；",
                        "5.修复发送按钮无法鼠标点击问题；",
                        "6.修复点击通知栏消息推送未跳转至对应聊天窗口的问题；",
                        "7.修复查看消息后，通知栏消息推送未消失的问题；",
                        "8.修复收到多次主动消息，仅推送第一次消息的问题；",
                        "9.修复在其他功能页面触发主动消息，返回后未刷新消息问题；",
                        "10.修复群聊昵称被私聊昵称覆盖问题；",
                        "11.修复群聊开启时间感知功能时，未注入时间的问题；",
                        "12.修复私聊关闭时间感知功能时，仍然注入时间的问题；",
                        "13.修复旧版群聊创建的角色无法绑定的问题，为避免发生同类问题，删除邀请新建角色方式，只保留邀请已有角色。"
                    ]
            },
            {
                    version: "Q.1.8",
                    date: "2026-07-16", 
                    notes: [
                        "1.新增api模型名称手动输入功能。当没有拉取到模型时，支持手工输入模型名称调用(此功能是本人薅京东云羊毛的时候增加的，拉不到模型可以试试)。",
                        "2.聊天列表新增设置侧边栏。",
                        "(1)“外观”入口整合入侧边栏；",
                        "(2)新增“消息通知”功能入口。",
                        "3.新增系统消息提示功能。",
                        "(1)设置入口在聊天列表右上角设置侧边栏，进入页面点击开启即可；",
                        "(2)新增自定义全局保活，开启后会有播放任务，这是保活在运行的正常现象。注意安卓手机需要将小手机开启自启动，以及关闭电池优化，具体品牌不同可能设置有所区别，但如果不开启，手机可能为了省电而杀掉后台保活中的OuO；",
                        "(3)新增通知内容设置功能；",
                        "(4)新增CF推送功能（实验），按照流程配置成功后，离线也可以接收消息。安卓需要科学上网，因为接的是谷歌推送，ios注意邮箱填写要加前缀。一般使用保活已经可以满足日常推送需求。CF部署文件及流程详见dc",
                        "4.新增版本显示功能。在日志可查看当前版本号。",
                        "5.功能优化",
                        "(1)新增聊天按钮合并：新增聊天、新增群聊、导入角色按钮合并为“+”按钮；",
                        "(2)侧边栏整合：新增设置侧边栏，外观入口整合入侧边栏；",
                        "(3)优化存储：减少内存占用，但因此会增加部分功能调用时长，包括不限于存储备份、聊天搜索、批量删除、总结等。上述功能加载大概会占用几秒，请耐心等待；",
                        "(4)删除聊天确认优化：删除聊天增加了二次确认，避免误删。",
                        "6.问题修复",
                        "(1)修复大文件导入页面崩溃问题。本次修改了备份文件的格式，请务必重新备份导出，原来的备份文件如果太大导入可能会崩溃；",
                        "(2)修复备份文件缺失读书笔记、测试任务数据的问题；",
                        "(3)修复自动云备份失效问题；",
                        "(4)系统状态栏已经有电池显示了，因此将主页重复的电池组件删除；",
                        "(5)修复点击输入框输入时，滑动会呼出专注世界书的问题；",
                        "(6)修复长期总结时，短期总结修改未生效的问题。"
                    ]
                },
            {
                    version: "Q.1.7",
                    date: "2026-06-27",
                    notes: [
                        "1.新增学习app。",
                        "(1)可通过home页ToDouo进入；",
                        "(2)学习功能首页“专注”功能为原“番茄钟”功能入口，“阅读”“测试”为新增功能；",
                        "(3)首页右上设置角可以设置api及首页称呼、打招呼内容。",
                        "2.新增阅读功能(可以和char一起读书了)。",
                        "(1)支持导入txt格式的书籍；",
                        "(2)点击左右区域翻页，点击中间部分可以呼出菜单；",
                        "(3)右上角设置功能：主要用于修改和char共读的各项设定；",
                        "(4)底部功能区域：进度条、搜索、书签、目录、总结等阅读功能；",
                        "(5)点击目录会显示目录和你已经加入的书签；",
                        "(6)点击char的头像会显示气泡框和输入栏。输入后可以与char讨论书本内容。(和聊天不一样，user发送后就会调用char的回复，所以想说什么一次性说完吧)。",
                        "3.新增测试功能(char批改你的考试)。",
                        "(1)首页分为“题库”和“测试任务”两个部分，需要先添加题库，才能点击右上角添加测试任务；",
                        "(2)“题库”支持ai根据书本生成、csv文件导入、单题添加三种模式；",
                        "(3)测试任务可以设定答题人、批改人、题库、世界书(这个人沉迷于把1+1答错让char点评)。测试任务页面可以查看过往答题记录。",
                        "4.问题修复",
                        "(1)修复可向量化记忆段数统计不准确的问题；",
                        "(2)修复向量化中断问题，跳过无法向量化的记忆段落；",
                        "(3)修复存量短期总结无法转化为记忆段落的问题；",
                        "(4)修复数据库重复问题；",
                        "(5)修复语音模式未注入世界书(后)的问题；",
                        "(6)修复日记时间不准确问题；",
                        "(7)修复长期总结编辑内容无法保存的问题；",
                        "(8)修复存储过大时容易页面崩溃问题；",
                        "(9)修复语音通话自动接听问题；",
                        "(10)修复后台消息无法自定义api的问题。"
                    ]
                },
                {
    version: "Q.1.6",
    date: "2026-04-24",
    notes: [
        "1.新增群聊主页。",
        "(1)可通过侧边设置栏进入；",
        "(2)支持直接点击编辑群名，点击头像可更换群头像；",
        "(3)新增统计栏，可查看 token 分布情况；",
        "(4)点击群聊成员头像可直接跳转对应角色主页；",
        "(5)群聊侧边设置菜单参照私聊侧边栏改造升级。",
        "2.时间感知入口从 API 设置迁移至聊天侧边栏，私聊与群聊均支持按对话单独开启或关闭，默认状态为【未开启】。⚠️ 之前在 API 设置中已开启时间感知的用户，本次更新后需重新进行设置。",
        "3.偷看手机功能优化。",
        "(1)备忘录详情界面支持 Markdown 渲染；",
        "(2)草稿箱改为增量生成，再也不用担心辛苦生成的内容被覆盖；草稿箱列表支持长按批量删除。",
        "4.API 设置重构。",
        "(1)各功能均可在功能内单独选择专属 API 预设，未选择时自动回退使用全局默认配置；",
        "(2)新增向量 API 配置项，目前用于记忆功能。",
        "5.新增向量记忆功能。向量记忆依赖短期总结，需先生成短期总结的摘要才可进行向量化，建议按日总结并核对摘要内容是否准确。",
        "(1)情绪浓度会影响记忆被回忆起的概率：希望角色牢牢记住的事请调高，日常琐事可手动调低；",
        "(2)颗粒度参数影响每段记忆片段内涵盖的对话轮数。其他参数可前往向量面板调整；",
        "(3)总结功能新增「按日期生成」与「空白总结」两种生成方式。",
        "6.番茄钟功能入口迁移至 ToDouo 的「专注」功能下，其他功能仍在完善中，暂不开放入口。",
        "7.修复了一些已知问题。",
        "(1)修复存在折叠通话消息时，消息搜索无法跳转到对应位置的问题；",
        "(2)修复时间感知提示词存在误导性的问题；",
        "(3)修复线上模式下发送的线下消息上下文内容异常的问题；",
        "(4)修复记忆档案功能卡顿的问题；",
        "(5)尝试修复 iOS 系统返回按钮显示异常的问题（修复效果存疑，如有问题欢迎反馈）。"
    ]
},
                {
                    version: "Q.1.5",
                    date: "2026-04-11",
                    notes: [
                        "1.新增通话功能。入口位于原偷看手机界面，通话结束后记录以折叠气泡形式保留在聊天页面，点击可展开查看详情。",
                        "2.聊天室侧边设置菜单精简调整：偷看手机入口迁移至设置菜单及 char 主页；原按钮位置更换为语音/视频通话功能入口。",
                        "3.底栏“我”页面升级为“通讯录”，char 与 user 的完整列表均可在此查看与管理。",
                        "4.新增 char 与 user 主页。",
                        "(1)可通过侧边设置栏或通讯录进入；",
                        "(2)支持直接点击编辑姓名、昵称、人设，点击头像可更换头像；",
                        "(3)char 主页新增统计栏，可查看 token 分布情况；user 主页额外支持点击编辑状态。",
                        "5.偷看手机功能：消息详情页新增屏蔽按钮，屏蔽后 AI 将不再读取该联系人信息，主要解决 AI 频繁生成特定联系人对话的问题。（如果还是生成了，那说明 char 真的很想和对方说话 >◡<）",
                        "6.修复了一些已知问题。",
                        "(1)修复应用启动时默认激活 Audio Session 的问题；",
                        "(2)修复撤回消息处理异常的问题；",
                        "(3)修复状态消息、引用消息未写入历史记录的问题；",
                        "(4)修复偷看手机界面残留上一个角色信息的问题；",
                        "(5)修复随机模式下后台消息重复投递的问题；",
                        "(6)修复状态有概率未能正常更新的问题。"
                    ]
                },
                {
                    version: "Q.1.4",
                    date: "2026-03-22",
                    notes: [
                        "1.优化了存储方式，现在不会因为数据过多而卡顿了！弹出存储迁移通知时，请备份以防万一。",
                        "2.聊天：",
                        "(1)增加了“继续回复”功能。user没有输入任何内容时，直接点击ai回复按钮即可触发弹窗提示，点击确认可以继续回复。",
                        "(2)后台消息增加“固定模式”，可设置回复时间和保活时间，到时间后调用api生成消息。",
                        "3.api设置增加“兼容模式”，使用中转站api报错400时可尝试开启，但不支持识图功能。",
                        "4.偷看角色手机功能：",
                        "(1)可添加联系人，生成时会读取联系人列表，为了防止生成时混乱，请提前清理删除原有对话后生成；",
                        "(2)unlock新增本地上传头像、编辑小号昵称、id，解决unlock时间轴混乱问题。",
                        "5.修复了一些已知问题。",
                        "(1)偷看手机数据导入后不显示的问题；",
                        "(2)备份缺失部分新增设置字段的问题；",
                        "(3)ai回复不触发时间感知的问题。"
                    ]
                },
                {
                    version: "Q.1.3",
                    date: "2026-03-17",
                    notes: [
                        "1.聊天增加了char【主动发消息】的功能。入口在“+”扩展菜单。切换到主动模式可设置消息发送频率和每日最大调用次数，用于调整char的发言频率。如果完全不需要收到角色消息，请设置为“免打扰模式”。默认是“随机模式”，不额外调用api随机发送消息。",
                        "2.修改了【偷看手机】功能，聊天、备忘录、购物车、中转站、浏览器、相册、unlock改为增量生成，长按可进入多选模式批量删除。",
                        "3.修改了时间感知功能的【时间显示方式】，用于适配主动发消息功能的时间戳。",
                        "4.增加【系统日志】查看功能，路径：设置-系统日志。",
                        "5.增加主页面聊天图标【未读消息角标】。",
                        "6.壁纸设置中新增【主页底部导航栏颜色适配】，只在网页直接打开中生效。",
                        "7.修复了一些已知问题：(1)修复群聊保存会触发时间感知的问题。(2)部分解决了发送按钮卡顿的问题。(3)修复论坛切换页面滚动位置错误问题。(4)修复消息列表不显示未读消息红点的问题。(5)修复壁纸设置中，修改主页顶部状态栏颜色时，会实时改变设置页面顶部状态颜色的问题。"
                    ]
                },
                {
                    version: "Q.1.2",
                    date: "2026-03-07",
                    notes: [
                        "1.聊天功能增加【外观】页面。可在这个页面查看自定义聊天室美化情况，支持自定义css，并可以通过基础面板按钮进一步修改。更换气泡请在原聊天设置中的气泡选框选择。",
                        "2.增加【屏幕自适应】功能，主要用于屏幕上的要素适应大屏幕。初始默认关闭，如果需要开启，请在主屏幕-底部设置图标-启用滑动返回-开启。",
                        "3.增加了【手势滑动返回】功能。初始默认关闭，如果需要开启，请在主屏幕-底部设置图标-启用滑动返回-开启。",
                        "4.【线下模式开启弹窗修改】，改为确认弹窗，原线下模式气泡css可在外观功能中直接修改。",
                        "5.修复了一些已知bug。"
                    ]
                },
                {
                    version: "Q.1.1",
                    date: "2026-02-28",
                    notes: [
                        "1.尝试适配了ios。可在设置界面选择是否需要上下避让。(这个功能对安卓没有用)",
                        "2.优化表情包功能。",
                        "(1)增加了分类，长按可以修改分类名字，一键关联，一键删除。",
                        "(2)增加了表情包关联，可以在char私聊窗口点击表情包，点击【关联】选择这个角色可以使用的表情包；群聊不支持单独关联，直接使用char私聊中关联的表情包。"
                    ]
                },
                {
                    version: "Q.1.0",
                    date: "2026-02-23",
                    notes: [
                        "【聊天】1.新增线下模式（仅限私聊）及全局搜索功能。",
                        "2.User人设现改为全局绑定模式，需先前往“我”页面新增和修改人设，其他所有涉及读取User人设的功能将统一采用此模式。",
                        "3.在群聊中修改昵称时，现会触发系统消息推送提示。",
                        "【世界书】新增“写作”分类。该分类不会作用于线上聊天，专用于线下模式、记忆档案、论坛等涉及长文字段落的场景。注意：“写作”分类在线下模式的提示词层级非常靠后，对生成结果的影响力显著高于“世界书（后）”。",
                        "【记忆档案】原“日记”功能修改为“记忆档案”。",
                        "1.新增“短期总结”与“长期总结”。角色日记不再直接加入聊天记忆，替换为总结内容为角色提供长时记忆。",
                        "2.日记与总结可分别关联不同的世界书，请前往界面重新设置关联。",
                        "3.支持“短期总结”与“日记”一键同步生成（注意：此操作需调用两次API，耗时较长，请耐心等待）。",
                        "【论坛】1.增加发帖与评论功能，并支持匿名互动。匿名默认用户名为“喵叽+四位数字”（数字可在“我”界面自定义修改，方便大家精分扮演不同人设调戏角色）。",
                        "2.发帖支持关联角色聊天记录，可自定义关联条数（建议适量，关联过多可能影响生成速度）。",
                        "3.新增【收藏】与【在看】功能，统一在底栏“收藏”页管理。【收藏】为私密仅自己可见；点亮【在看】则角色可见。提醒：与角色讨论完毕后记得取消【在看】，否则在你和其他角色聊天时，容易被意外翻旧账哦。",
                        "💡 论坛小贴士：建议在世界书或角色人设中明确写明“该角色网名是xxx”，以固定角色或NPC的回帖用户名。",
                        "【游戏】新增仿RPG游戏互动功能，带来更丰富的沉浸式体验。",
                        "【主页】把一些功能都整合到了底部“设置”中。",
                        "【其他】进行了大量零散细节的修改，因为比较零碎就不在日志里写了_(:з」∠)_"
                    ]
                },
                {
                    version: "1.3.0",
                    date: "2025-11-11",
                    notes: [
                        "务必仔细观看！重复观看指路→主屏幕的教程app→更新说明！",
                        "新增：双语模式，位于聊天界面的侧边栏内，当char为外国人而你想要更沉浸式的对话时，可按需开启，开启后会将“外文（中文）”的消息识别成双语消息气泡，注意！中文翻译必须在括号内，点击气泡后展开翻译。",
                        "↑补充：如果掉格式如何修改？编辑窗口将翻译括号内的其他括号删掉，比如带括号的颜文字、(笑)等。如果还有bug就先别开这个吧！",
                        "新增：流式传输开关，位于api设置界面，开跟不开不知道有什么区别，总之做了嗯嗯。没改之前默认是流式传输，如果非流出不来就开流式，流式出不来就关流式，都出不来我也没招了！",
                        "新增：拓展css美化代码，指路→主屏幕的自定义app→全局美化css中，下滑。有隐藏头像、底部变简洁、隐藏表情包背景之类的，可自行复制需要的部分放到全局美化中。",
                        "补充教学：发现有些宝宝还有地方不太清楚怎么使用，补充一下",
                        "1. 偷看手机：存储为临时存储，离开char手机页面后，偷看的相关内容将会清空，有喜欢的内容请及时截图",
                        "2. 回忆日记：生成日记后，需点亮该篇日记右上角的☆按钮收藏，收藏后该篇日记才会作为char的回忆加入聊天上下文中",
                        "3. 日记使用拓展方法：日记内容可编辑，当日记篇数过多/char被日记内的主观形容影响性格较大时，可以将你需要保留的日记内容复制给某个ai（豆包、deepseek、哈吉米都行）进行大总结，指令参考：以全客观的、不参杂任何主观情绪，以第三人称视角按照时间顺序总结发生过的事件和关键语句。然后将返回的总结塞进日记收藏加入上下文即可。",
                    ]
                },
                {
                    version: "1.2.0",
                    date: "2025-10-15",
                    notes: [
                        "新增：猫箱图床 (Catbox) 渲染机制，在当前绑定的表情包世界书中包含 'catbox' 关键词即可切换到猫箱模式，注意！iposting图床表情包和猫箱表情包不可同时渲染，只能选择一方。如：绑定了猫箱表情包世界书，就无法渲染过往iposting图床的表情包，不绑定则反之。",
                        "新增：世界书批量删除功能，长按条目即可进入多选删除模式，支持分类全选。",
                    ]
                },
                {
                    version: "1.1.0",
                    date: "2025-10-13",
                    notes: [
                        "重要！！已更换存储方式，请尽快导出原网址的备份并清理浏览器内该网址的数据，并重新在此网址导入备份",
                        "新增：番茄钟，可以创建专注任务并绑定char和自己的人设预设（仅可从预设中选择），在列表中左滑删除任务。专注期间想摸鱼了可以戳一戳头像，ta会对你做出回复。每个专注界面的设置键可以自定义鼓励频率和限制自己戳一戳的次数，超过次数则ta不会再理你，请补药偷懒，努力专注吧！",
                        "新增：两个桌面小组件，现所有小组件都可以通过点击来自定义图片和文字",
                        "优化：修复了存储膨胀的问题，现为测试阶段不确定是否有bug，请勤备份！唯有备份才是安全的！",
                        "优化：修复了一些使用体验上的小问题",
                        "画饼（未来可能会做的）：1.第二页的布局美化 2.日记本、存钱罐、音乐",
                    ]
                }
            ];
              
                                          // --- NEW: Update Log Functions ---
                function renderUpdateLog() {
                    const tutorialContent = document.getElementById('tutorial-content-area');
                    if (!tutorialContent) return;

                    const updateSection = document.createElement('div');
                    updateSection.className = 'tutorial-item'; // Use tutorial-item class, default open

                    let notesHtml = '';
                    updateLog.forEach((log, index) => {
                        notesHtml += `
                        <div style="margin-bottom: 15px; ${index < updateLog.length - 1 ? 'padding-bottom: 10px; border-bottom: 1px solid #f0f0f0;' : ''}">
                            <h4 style="font-size: 15px; color: #333; margin: 0 0 5px 0;">版本 ${log.version} (${log.date})</h4>
                            <ul style="padding-left: 20px; margin: 0; list-style-type: '› ';">
                                ${log.notes.map(note => `<li style="margin-bottom: 5px; color: #666;">${note}</li>`).join('')}
                            </ul>
                        </div>
                    `;
                    });

                    updateSection.innerHTML = `
                    <div class="tutorial-header">更新日志</div>
                    <div class="tutorial-content" style="padding-top: 15px;">
                        ${notesHtml}
                    </div>
                `;

                    tutorialContent.appendChild(updateSection);
                }

                function showUpdateModal() {
                    const modal = document.getElementById('update-log-modal');
                    const contentEl = document.getElementById('update-log-modal-content');
                    const closeBtn = document.getElementById('close-update-log-modal');

                    const latestLog = updateLog[0];
                    if (!latestLog) return;

                    contentEl.innerHTML = `
                    <h4>版本 ${latestLog.version} (${latestLog.date})</h4>
                    <ul>
                        ${latestLog.notes.map(note => `<li>${note}</li>`).join('')}
                    </ul>
                    <p style="font-size: 12px; color: #888; text-align: center; margin-top: 15px; border-top: 1px solid #eee; padding-top: 10px;">过往更新说明可在“教程”应用内查看。</p>
                `;

                    modal.classList.add('visible');

                    closeBtn.onclick = () => {
                        modal.classList.remove('visible');
                        localStorage.setItem('lastSeenVersion', appVersion);
                    };
                }

                function checkForUpdates() {
                    const lastSeenVersion = localStorage.getItem('lastSeenVersion');
                    if (lastSeenVersion !== appVersion) {
                        // Use a small delay to ensure the main UI has rendered
                        setTimeout(showUpdateModal, 500);
                    }
                }
                
  let loadingBtn = false;